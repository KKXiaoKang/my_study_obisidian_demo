...
> **总结 Step 1**：用一个左右手分别配置的固定旋转 \(R_{12,s}\)，把 VR 手掌系刚性重对齐到机器人末端系；再叠一个经验 bias 修补手跟踪误差。
...
> **总结 Step 2**：把胸部 yaw 解耦掉，使手姿态变成"相对躯干前向的姿态"，避免操作员转身时机器人手抖动跟随。
...
> **一句话总结**：anchor 是"VR 端的局部零点"，target 是"机器人端的累积目标"。每帧把 VR 的位姿增量直接累加到 target，让操作员可以"摆姿势 → 松手 → 重新摆"，机器人手只会跟着相对运动走，不会因为 VR 绝对坐标和机器人绝对坐标不重合而漂移。
...
> 这套 retarget 用的是分段尺度映射：上臂向量 v_u 和前臂向量 v_l 各自按机器人臂长比例缩放后再串联重建肘和手。这个方案在工程上有一个核心缺陷——约束的非对称作用。我们对 v_u 施加了三层运动学约束（角速度截断、跨胸内收旋转、锥形可达投影），但 v_l 完全没有被同步旋转。结果就是：当人臂物理伸直、v_u 与 v_l 共线时，约束修改 v_u 方向之后，机器人侧的肩→肘和肘→手方向不再共线，IK 出来的机器人手臂被几何性地"掰弯"。"前伸 vs 侧平举二选一"的现象本质是：锥形约束的 reference_vec 是 (1, ±0.3, -0.1)，对前伸方向（+X）几乎不触发，对侧平举方向（±Y）必然触发。你调约束参数让某一种姿态不触发约束，另一种就一定触发，无解。修复办法是把作用在 v_u 上的等效旋转 R_C 同步施加到 v_l，或者改用关节空间 retarget 从根本上避开这个问题。